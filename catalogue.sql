...
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
	...
	1)  SELECT * FROM film WHERE date_sortie >= '1960-01-01'
SELECT * FROM film WHERE date_sortie < '1960-01-01'

	2)  UPDATE `film` SET `description` = 'salut' WHERE `film`.`id` = 35

	3)  INSERT INTO `film` (`id`, `titre`, `slug`, `image`, `prix`, `description`, `date_sortie`) VALUES ('99', 
	'One Piece Red', 'Oda', 'https://sm.ign.com/ign_fr/screenshot/default/unnamed_am6r.jpg', '30', 
	'meilleur film One Piece', '2022-10-13')
	      
	INSERT INTO `films_categs` (`id_film`, `id_categorie`) VALUES (99, 35), (99, 52), (99, 49)

	4)  SELECT DISTINCT * FROM commentaire WHERE id_film = 43 

	5)  DELETE FROM film WHERE slug LIKE 't%'

	6)  SELECT DISTINCT user.* FROM commentaire, user WHERE user.id = commentaire.id_user

	7)  SELECT * FROM films_categs WHERE id_categorie = 51
 
	8)  SELECT AVG(note) FROM commentaire	(3,8571)

	9)  SELECT * FROM film WHERE MONTH (date_sortie) = 6

	10) SELECT DISTINCT user.* FROM user INNER JOIN commentaire ON commentaire.id_user=user.id WHERE user.role=1